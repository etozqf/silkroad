...
$.editplus.setPlugin('db',function(a,sel){
    ...
    function(dialog){
        ...
        var dsn = $(frm.dsnid);
        var prefix = $(frm.prefix);
    	var version = $(frm.version).change(function(){
    		$.getJSON('?app=system&controller=template&action=load_discuz', {
                dsnid: dsn.val(),
                version: this.value,
                prefix: prefix.val()
            }, function(json){
    		...
    		$.post('?app=system&controller=template&action=save_tag_discuz_options', {
                dsnid: dsn.val(),
                version: this.value,
                prefix: prefix.val()
            });
    	...
    	if (frm.dsnid && frm.dsnid.value && frm.dsnid.value != '0' && frm.version.value && frm.version.value != '0') {
            version.change();
    	}
    }, function(dialog){
    	var frm = dialog.find('form')[0];
    	var t = '{discuz';
    	// dsnid dsnname
        if (frm.dsnid && frm.dsnid.value && frm.dsnid.value != '0') {
            t += ' dsn="'+frm.dsnid.options[frm.dsnid.selectedIndex].text+'"';
        }
        if (frm.version && frm.version.value && frm.version.value != '0') {
            t += ' version="'+frm.version.options[frm.version.selectedIndex].value+'"';
        }
        frm.prefix && (t += ' prefix="'+frm.prefix.value+'"');
        frm.filter && (t += ' filter="'+$(frm.filter).filter(':checked').val()+'"');
        var specials = [];
        frm.special && $(frm.special).each(function(){
        	this.checked && specials.push(this.value);
        });
        if (specials.length) {
        	t += ' special="'+specials+'"';
        }
        if (frm.published && frm.published.value && frm.published.value != '0') {
        	t += ' published="'+frm.published.value+'"';
        }
        frm.orderby && (t += ' orderby="'+frm.orderby.value+' '+$(frm.ascdesc).filter(':checked').val()+'"');
        ...
        if (frm.keywords && frm.keywords.value) {
        	t += ' keywords="'+frm.keywords.value+'"';
        }
        var size = frm.size && parseInt(frm.size.value);
        if (size) {
            t += ' size="'+size+'"';
        }
