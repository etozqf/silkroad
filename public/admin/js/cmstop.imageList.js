...
imageList.prototype = {
    ...
    imageUrl: function() {
        var R_PREFIX_UPLOAD_URL = new RegExp('^'+UPLOAD_URL),
            R_PREFIX_ABSOLUTE_URL = /^https?:\/\//i;
        return function(f, abs) {
            if (! f) return f;
            if (! abs) {
                return f.replace(R_PREFIX_UPLOAD_URL, '');
            }
            if (R_PREFIX_UPLOAD_URL.test(f)) {
                return f;
            }
            if (R_PREFIX_ABSOLUTE_URL.test(f)) {
                return f;
            }
            return UPLOAD_URL + f;
        };
    ...
    addLocalImage: function(data, moreToAdd) {
        ...
        if (this.localImageList.find('li.image-thumb-item').filter(function(index) {
            return ($(this).attr('aid') == data.aid) && (data.aid != 0);
        }).length) {
        ...
        editButton.click(function(e) {
            ...
            var win = window,
                ...
                afterEdit = function(json) {
                    imageField.val(self.imageUrl(json.file));
        ...
        descArea.bind('click', function(e) {
            ...
            $(document).bind('click.editDesc', function(e) {
                if (e.target != textarea.get(0) && $(e.target).parents().index(item.get(0)) == -1) {
                    desc = item.find('input[data-role=desc]').val();
                    item.trigger('saveEditDesc');
                }
            });
        });
    ...
    renderLocalImage: function(data) {
        ...
        var item = $([
            '<li class="image-thumb-item" aid="', data.aid, '">',
                ...
                '<input type="hidden" data-role="thumb" name="post[', data.aid, '][thumb]" value="', this.imageUrl(data.thumb) || '', '" />',
                '<input type="hidden" data-role="image" name="post[', data.aid, '][url]" value="', this.imageUrl(data.image), '" />',
                ...
                '<a data-role="thumb-show" class="image-thumb-item-a" href="javascript:;"', (this.options.dragsort ? ' title="拖动以排序"' : '') , '></a>',
    ...
    updateThumbShow: function(item, thumb) {
        this.loadImage(this.imageUrl(thumb, true) + '?' + Math.random(), function(url) {
    ...
    renderToEditor: function(src, desc) {
        ...
        var html = '<p data-mce-style="text-align: center;" style="text-align:center;"><img src="' + this.imageUrl(src, true) + '"' + (desc ? ' alt="' + desc + '"' : '') + ' /></p>';
