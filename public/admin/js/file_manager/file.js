...
function edit_file()
{
    ...
    function afterEdit(json) {
        $.getJSON('?app=system&controller=image&action=thumb&width=100&height=100&file=' + json.file, function(json) {
            ...
            var thumb = UPLOAD_URL + json.thumb;
            li.find('div.icon img').attr('src', thumb + '?' + Math.random() * 9);
            li.attr('thumb', thumb);
            li.attr('src', UPLOAD_URL + json.file);
        });
    }
