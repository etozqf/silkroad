	...
	window.Media = {
		init : function() {
			$("#uploadvideo").uploader({
				...
				{
					switch (response.split('.').pop()) {
						case 'swf':
						case 'flv':
							$('#media_type')[0].value = 'flash';
							break;
						case 'wmv':
						case 'wmp':
							$('#media_type')[0].value = 'windowsmedia';
							break;
						case 'qt':
							$('#media_type')[0].value = 'quicktime';
							break;
						case 'rm':
						case 'rmvb':
						case 'rmp':
							$('#media_type')[0].value = 'realmedia';
							break;
					}
					$('#src').val(response);
				}
		...
		getMediaTypeHTML : function(editor) {
			function option(media_type, element) {
				...
				return '<option value="'+media_type+'">'+media_type+'</option>'
			}
