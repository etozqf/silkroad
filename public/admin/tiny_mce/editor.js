...
var EDITOR_OPTIONS = {
    ...
    flash_video_player_url: IMG_URL + 'js/player/cmstopPlayer.swf',
    flash_video_player_flashvars: {file:'$url'},
	...
	setup : function(ed){
        ...
        ed.onMouseDown.add(function(ed, evt) {
            ...
            container.onmousedown && container.onmousedown();
            ('jQuery' in window) && $(container).trigger('mousedown');
        });
        ed.onClick.add(function(ed, evt) {
            var container = ed.getContainer();
            container.onclick && container.onclick();
            ('jQuery' in window) && $(container).trigger('click');
        });
	}
};
function editor(textarea_id, type, options){
	switch(type){
	case 'complete':
		ed_plugins = "safari,pagebreak,style,advimage,advlink,preview,contextmenu,paste,template,inlinepopups,onekeyclear,fullscreen,ct_media,media,table,ct_sdbc,ct_moreread,ct_tag,ct_taglink,ct_vote,ct_modeswitch,ct_source,ct_save,searchreplace,ct_wdcount,ct_image,ct_template";
		...
		ed_theme_advanced_buttons2 = "charmap,ctWdcount,ctSdbc,pastetext,pasteword,replace,ctImage,media,ctVote,ctMoreread,ctTag,ctTaglink,onekeyclear,pagebreak,ctSource,fullscreen,ct_template,ctModeswitch";
		ed_theme_advanced_buttons3 = 'tablecontrols';
		break;
	case 'mini':
	...
	case 'none':
		...
		break;
	default:
		ed_plugins = "safari,pagebreak,style,advimage,advlink,insertdatetime,preview,searchreplace,contextmenu,paste,template,inlinepopups,onekeyclear,fullscreen,ct_media,media,insertimage,ct_sdbc,ct_moreread,ct_tag,ct_vote,ct_modeswitch,ct_save,ct_wdcount,ct_image";
		ed_theme_advanced_buttons1 = "undo,bold,italic,underline,fontsizeselect,forecolor,|,link,unlink,|,justifyleft,justifycenter,justifyright,|,ctImage,ctMoreread,ctTag,ctTaglink,onekeyclear,pagebreak,fullscreen,ctModeswitch";
		ed_theme_advanced_buttons2 = '';
		ed_theme_advanced_buttons3 = '';
		break;
	}
