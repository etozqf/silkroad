...
var App = {
	init:function(){
		...
		$replacement.find('>tr:gt(0)').each(function(){
			var tr = $(this);
		...
		var replaceAdder = $('<a href="javascript:;">添加</a>').click(function(){
		...
		$replacement.find('>tr:first>td:last').append(replaceAdder);
		$(form).ajaxForm(function(json){
			if (json.state) {
				ct.confirm(json.info+'，继续编辑此规则？',null,function(){
					ct.assoc.close();
				});
			} else {
				ct.error(json.error);
	...
	testGetList:function(){
		var data = [
            ...
            'listNextPageStart='+encodeURIComponent(form.listNextPageStart.value),
            'listNextPageEnd='+encodeURIComponent(form.listNextPageEnd.value)
		].join('&');
	...
	testGetDetail:function(){
		var data = [
            ...
            'nextPageStart='+encodeURIComponent(form.nextPageStart.value),
            'nextPageEnd='+encodeURIComponent(form.nextPageEnd.value),
			...
			'tagStart='+encodeURIComponent(form.tagStart.value),
			'tagEnd='+encodeURIComponent(form.tagEnd.value),
			'tagSeparater='+encodeURIComponent(form.tagSeparater.value),
			'descriptionStart='+encodeURIComponent(form.descriptionStart.value),
			'descriptionEnd='+encodeURIComponent(form.descriptionEnd.value),
