...
_add_form_ready = function(form, dialog) {
	...
	dialog.find('input[name=type]:not(:checked)').click(function(){
		var n = f.name.value, c = f.description.value, d = f.data.value;
...
editor = {
    init:function(id) {
        ...
        $.editplus.setPlugin('clip',function(a,sel){
            ...
            var pageName = $.trim(pageDropdown.find('option[value='+id+']').text());
	    	var title = '添加/转化区块到页面：' + pageName;
            ct.formDialog({
                title:title,
                width:540
            }, '?app=page&controller=section&action=add&pageid='+id, function(json) {
                if (json.state) {
                    id == selectedPageid && editor.addOneSection(json.data);
                    insertText(editEl[0], '<!--#include virtual="/section/'+json.data.sectionid+'.html"--><!--# '+pageName+' '+json.data.name+' #-->', sel.selection);
                    return true;
                }
		...
		ep = editEl.editplus({
			buttons:'fullscreen,wrap,|,db,content,discuz,phpwind,shopex,|,loop,ifelse,elseif,template,|,section,clip',
			width:430,
			height:430
		});
        ...
        var a = li.find('a');
        var pageName = $.trim(pageDropdown.find('option[value='+(selectedPageid || pageid)+']').text());
    	a.click(function(){
        	insertText(editEl[0], '<!--#include virtual="/section/'+item.sectionid+'.html"--><!--# '+pageName+' '+item.name+' #-->');
        	return false;
    	...
    	$.getJSON('?app=page&controller=page&action=sections&pageid='+id+'&status=1',
		function(json){
			for (var i=0,item;item = json.data[i++];editor.addOneSection(item)){}
		});
