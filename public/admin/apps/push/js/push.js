...
var viewedOne = function(id, li)
{
	if (!(li.hasClass('viewed') || li.hasClass('pushed')))
	{
		...
		$.post('?app=push&controller=push&action=viewedone&recommendid='+id,
		function(){
			json.status = 'viewed';
			li.addClass('viewed');
...
function pushItem(taskRow, callback)
{
	...
	var data = 'recommendid='+id+'&catid='+taskRow.find('input').val()+'&status='+status;
...
var App = {
	init:function(cat){
		...
		ulAll = new scrollFeed(ul[0], {
			template:'\
		<li id="item_{recommendid}" param="recommendid={recommendid}" class="{status}">\
