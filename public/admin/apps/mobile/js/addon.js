...
function genPlaces(data, callback) {
    $.post('?app=mobile&controller=addon&action=genPlaces', data, function(json) {
        if (json && json.state) {
            callback(json);
        } else {
...
var MobileAddon = {
    ...
    render: function(container, contentid) {
        ...
        container.load('?app=mobile&controller=addon&action=render' + (contentid ? ('&contentid=' + contentid ): ''), function() {
    ...
    initialize: function() {
        ...
        this.container.find('.addon-engine').click(function() {
            ...
            if (MobileAddon.readAddon(engine)) {
    ...
    addAddon: function(engine, trigger) {
        var addonEngine = MobileAddon.engines[engine], form;
        if (actionLock || ! addonEngine) {
            return false;
        }
        actionLock = true;
        trigger.dropdialog({
            content: '?app=mobile&controller=addon&action=addAddon&engine=' + engine,
    ...
    editAddon: function(engine, addonid, trigger) {
        ...
        actionLock = true;
        trigger.dropdialog({
            content: '?app=mobile&controller=addon&action=editAddon&engine=' + engine + (addonid ? ('&addonid=' + addonid) : ''),
            width: addonEngine.dialogWidth || 350,
            region: region,
            closeOnBlur: false,
            buttons: {
            ...
            afterRender: function(dialog) {
                ...
                $('<a class="addon-delete" href="javascript:void(0);" title="删除">删除</a>').appendTo(dropdialog.extra).click(function() {
                    MobileAddon.removeAddon(engine, trigger);
                    dropdialog.close();
                    return false;
                });

                genPlaces({
                    contentid: MobileAddon.contentid,
                    engine: engine,
                    addonid: addonid
                }, function(json) {
                    MobileAddon.readAddon(engine) && (json.selected = MobileAddon.readAddon(engine).place);
                    renderPlaces(json, function(select) {
                        $(select).appendTo(dropdialog.extra).selectlist({
                            direction:'up'
                        });
...
window.MobileAddon = MobileAddon;
})(jQuery);