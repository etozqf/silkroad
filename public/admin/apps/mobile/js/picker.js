...
function query(params, clear) {
    ...
    $.post('?app=mobile&controller=content&action=picker', params, function(json) {
        if (json && json.data) {
            message.hide();
            total = json.total;
            if (clear) {
            ...
            $.each(json.data, function(index, item) {
                var input,
                    row = $([
                        '<div id="picker-content-'+item.contentid+'" class="picker-panel-item">',
                            '<span class="time">'+item.time+'</span>',
                            '<input type="'+(multiple ? 'checkbox' : 'radio')+'" name="item[]" />',
                            '<a href="', item.url, '" target="_blank" title="查看"><div class="model-icon model-icon-', item.model, '"></div></a>',
