...
var tpl = $.trim('<li>\
					...
					<a href="{%gusturl%}" class="talk"></a>\
					<span class="fav" catid="{%catid%}" id="collect_{%contentid%}" contentid="{%contentid%}">收藏</span>\
				</li>');
$.fn.itemSearch = function(itemUrl,data){
	if(!data){data=""};
	var itemUrl = itemUrl+data+"&pagesize=20&page=%p&minsum="+$('[name=min]').val()+"&maxsum="+$('[name=max]').val()+"&currency="+$('[name=currency]').val()+"&jsoncallback=?"
    // // 分页查询数据
    $("#tablelist").tablelist({
      baseurl:itemUrl,
      pager:".page",
