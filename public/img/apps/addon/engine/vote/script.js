...
function formatItem(data, callback) {
    $.post('?app=addon&controller=addon&action=vote_format', data, function(json) {
        callback(json);
    }, 'json');
}
...
function bindEvents(dialog) {
    ...
    dialog.find('[data-role=add]').click(function() {
        ct.assoc.open('?app=vote&controller=vote&action=add&catid='+$('#catid').val(), 'newtab');
        return false;
    });
}
...
fet('cmstop lib.tree', function() {
    Addon.registerEngine('vote', {
        dialogWidth: 650,
        afterRender: function(dialog) {
            var storedData = Addon.readAddon('vote');
        ...
        editFormReady: function(form, dialog) {
            fet('admin.datapicker admin.uploader', function() {
                var addon = Addon.readAddon('vote'),
