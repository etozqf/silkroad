 ...
 * @version    $Id: cmstop.tree.js 7855 2013-01-08 06:38:08Z liyawei $
 */
(function($){
var OPTIONS = {
...
tree.prototype = {
	branches:function(li, callback){
		var t = this, o = t.options, item = li.data('item');
		li.data('locked', true);
		$.ajax({
			url:(item && item[o.paramChildMenu] || o.url).replace('%s', encodeURIComponent(li.attr('idv'))),
...
var selectree = function(elem, options){
	...
	$elem.bind('changed initd', function(ev, _checked, _storedData){
        checked = _checked;
		o.render(checked, _storedData, a, o);
		if (valBox) {
	...
	function initTree() {
		...
		new $.tree(tree, {
			...
			renderTxt:function(div, val, item){
				var	label;
				if ((o.ending && item[o.paramHaschild]) || val.charAt(0) == '*') {
