...
(function ($) {
    ...
    $.fn.colorPicker = function (vIn) {
        ...
        if (cp.length == 0) {
            ...
            cp.css({"position":"absolute", "z-index":"10000", "background-color":"#FFFFFF", "border":"1px solid #CCCCCC", "padding":"1px", "cursor":"pointer", "display":"none"});
            ...
            for (r = 0; r < 6; r++) {
                for (g = 0; g < 6; g++) {
                    for (b = 0; b < 6; b++) {
                        ...
                        s[j + 1] = '<td class="color" height="10" bgColor="#' + c + '" width="10" style="width:10px;background-color:#' + c + ';"></td>';
                        i++;
                        j++;
                    }
                }
            }
            s[j + 1] = '</tr><tr><td height="10" colspan="16" id="fy_ColorPicker_Select" style="font-family:Tamoha;font-size:10px;text-align:center;cursor:default;"></td><td title="清除" class="color" bgcolor="" height="10" colspan="1" title="Empty" align="center" style="font-family:Tamoha;font-size:10px">E</td><td title="透明" class="color" bgcolor="transparent" height="10" colspan="1" title="Transparent" align="center" style="font-family:Tamoha;font-size:10px">T</td></tr></table>';
            cp.html(s.join(''));
        ...
        if (cp.is(':visible') && tl.Top == loc.Top && tl.Left == loc.Left) {
            ...
            var doc = $(document).click(function (e) {
                var t = e.target;
                if (cp.is(':visible') && !compareObj(t) && t != cp[0] && cp.find('*').index(t) == -1) {
                    cp.hide();
                    if ($.browser.msie) cf.hide();
                }
                doc.unbind('click', arguments.callee);
            });
    ...
    function bindEvent(elem, cp) {
        $(document).bind('mousedown.colorPicker', function (ev) {
            var target = ev.target;
            if (target != elem.get(0) && target != cp.get(0) && $(target).parents().index(cp.get(0)) === -1) {
                cp.hide();
            }
        });
    }
    ...
    function GetLoc(element) {
        ...
        var offset = (element.jquery ? element : $(element)).offset();
    ...
    $.fn.titleColorPicker = function (titleSl, colorSl) {
        var el = this;
        this.click(function () {
            el.colorPicker({
                setColor:titleSl,
