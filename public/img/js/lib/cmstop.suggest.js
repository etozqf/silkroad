 ...
 * @version    $Id: cmstop.suggest.js 10709 2015-04-23 06:49:23Z chenchao $
 */
(function($){
var LANG = {
	removeItem:'移除此项',
...
function suggest(elem, options) {
	...
	list = new List($.extend(listOptions, {
		...
		click:function(checked, val, item){
			if (!checked) {
				removeItem(item[options.paramVal]);
				return;
			}
			if (!options.limit || selectedValue.length < options.limit) {
				addItem(item);
			} else {
				return false;
			}
	...
	function addItem(item) {
		...
		valBox.val(selectedValue.join(options.delimiter));
		list.check(val);
	}
	function txtItem() {
	...
	if (val !== '') {
		if (options.initUrl) {
			...
			var data = val.split(new RegExp(options.delimiter));
	...
	self.addItem = function(item) {
		if(item){
			$.map(item.split(options.delimiter), function(item) {
				addItem({'tag':item});
			});
		}
	};
	self.txtItem = txtItem;
	self.removeItem = function(a) {
		if (typeof a == 'string') {
			a = $('a.selectedItem[name='+a+']')
		}
		removeItem(a);
	}
}
...
window.suggest = function(elem, options) {
	return new suggest(elem, options);
}
})(jQuery);
...
function loadPage(t, callback){
	...
	$.ajax({
		...
		success:function(json) {
            ...
            $.isFunction(callback) && callback(t, json);
		},
		complete:function(){
			t._lock = false;
		},
...
function buildItem(item, t){
	...
	label.find('input').click(function(){
		if (o.click(this.checked, val, item) === false) return false;
...
$.suggest.registerBridge('scroll', function(t){
    ...
    function loadAdjust(instance, json) {
        if (instance._count >= json.total) {
            return;
        }
        var last = container.children(':last');
        if (last.length && (last.position().top + last.height() < container.innerHeight())) {
            loadPage(t, loadAdjust);
        } else {
