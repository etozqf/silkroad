 ...
 * @version    $Id: cmstop.autocomplete.js 7013 2012-09-21 02:06:43Z shaoshengqiang $
 */
(function($){
var OPTIONS = {
...
function autocomplete(input, options) {
	// xiaoqiang
	if(options.CLASSES) $.extend(CLASSES,options.CLASSES);
	// @end xiaoqiang 
    ...
    function display(data, k) {
    	...
    	blurCurrent();
    	div.empty().css('height', data.length > 10 ? options['css']['height']||200 : 'auto');
    	options.displayAfter(div);
    ...
    function addItem(item, k){
    	var text = typeof options.itemFormat == 'function'
    		? options.itemFormat(item, k) : item[options.textFiled];
    	var a = $('<a>'+text+'</a>')
		.mousedown(function(){
			selectItem(a);
			return false;
    ...
    function activeItem(a, item){
    	...
    	options.autoFill && input.val(item[options.textFiled]);
    	options.activeAfter(a,item,input);
    }
    function selectItem(a){
    	...
    	options.selectAfter(a,a.data('itemdata'),input);
    }
}
