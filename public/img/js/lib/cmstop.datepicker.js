...
var document = window.document,
	...
	rzero = /^0/,
	rhastime = /([Hhms])\1*/,
	rroot = /^body|html$/i, boxModel = document.compatMode == 'CSS1Compat',
...
function mousewheel(elem, fn){
	...
	if (elem.addEventListener) {
		...
		Event.add(elem, 'DOMMouseScroll', handle);
	} else {
		Event.add(elem, 'mousewheel', handle);
	}
}
...
var DatePanel = function(){
	...
	function numbox(count, elem, place){
        var box = null;
        function init(){
            var frag = [];
            for (var i=0; i < count; i++) {
                frag.push('<a hideFocus href="" class="'+CLASSES.NUM+'">'+pad(i, 2)+'</a>');
            }
            box = appendTo('<div class="'+CLASSES.NUMBOX+'">'+frag.join('')+'</div>', place);
        ...
        function hide(){
            Event.remove(document, 'mousedown', blur);
            css(box, 'display', 'none');
        }
        function show(){
            box || init();
            css(box, 'display', 'block');
            Event.add(document, 'mousedown', blur);
        }
        function select(){
            elem.select();
        }
        function blur(e){
            contains(box, e.target) || contains(elem, e.target) || hide();
        }
        Event.add(elem, 'keydown', function(e){
            ...
            // UP DOWN
            case 38: case 40:
                elem.value = pad(Math.max(Math.min(toInt(elem.value) + 39 - e.which, count-1), 0), 2);
                select();
                e.halt();
                return;
            // BACKSPACE TAB ENTER ESC LEFT RIGHT DELETE
            case 8:case 9:case 13:case 27:case 37:case 39:case 127:
                return;
            }
        ...
        Event.add(elem, 'focus', select);
        Event.add(elem, 'dblclick', function(){
            select();
            show();
        });
    }
	function DatePanel(options) {
		...
		var head = appendTo(
			'<div class="'+CLASSES.HEAD+'">'+
				'<a hideFocus href="" class="'+CLASSES.CTRL+' '+CLASSES.PREV+'">&#x00AB;</a>'+
				'<a hideFocus href="" class="'+CLASSES.CTRL+' '+CLASSES.UP+'"></a>'+
				'<a hideFocus href="" class="'+CLASSES.CTRL+' '+CLASSES.NEXT+'">&#x00BB;</a>'+
			'</div>', wrapper);
		var body = appendTo(
			...
			t._timeBox = find(appendTo(
				'<div class="'+CLASSES.TIME+'">'+
					...
					'<span class="'+CLASSES.TIME+'-value">'+
						'<input class="'+CLASSES.TIME+'-h" value="'+hours+'" /><b>:</b>'+
						'<input class="'+CLASSES.TIME+'-m" value="'+minutes+'"/><b>:</b>'+
						'<input class="'+CLASSES.TIME+'-s" value="'+seconds+'"/>'+
					'</span>'+
				'</div>', foot),
			'input');

			numbox(24, t._timeBox[0], wrapper);
			numbox(60, t._timeBox[1], wrapper);
			numbox(60, t._timeBox[2], wrapper);
		}
		if (o.buttons) {
			var buttonArea = appendTo('<div class="'+CLASSES.BUTTON+'"></div>', foot), buttons = [];
			for (var name in o.buttons) {
				buttons.unshift('<a hideFocus name="'+name+'" class="'+CLASSES.ITEM+'" href="">'+name+'</a>');
			}
	...
	DatePanel.prototype = {
		_renderDateItem:function(year, month, date, flag) {
			...
			return '<a hideFocus href="" class="'+cls+'" date="'+val+'">'+date+'</a>';
		},
		_renderMonthItem:function(year, month) {
			return '<a hideFocus href="" class="'+CLASSES.ITEM+'" date="'+year+'-'+month+'">'+LOCALE.MONTHNAME[month]+'</a>';
		},
		_renderYearItem:function(year){
			return '<a hideFocus href="" class="'+CLASSES.ITEM+'" date="'+year+'">'+year+'</a>';
		},
...
var DatePicker = function(){
	...
	function pos(block, box) {
		var sT = ('pageXOffset' in window) ? window.pageXOffset :
				boxModel && document.documentElement.scrollTop || document.body.scrollTop,
			sL = ('pageYOffset' in window) ? window.pageYOffset :
		...
		if (off.top - sT + block.offsetHeight + box.offsetHeight > iH) {
			box.style.top = Math.max(sT, sT + iH - box.offsetHeight, off.top - box.offsetHeight) + 'px';
		} else {
	...
	DatePicker.prototype = {
		...
		_getCapture:function(e){
			...
			return t && (contains(w, t) || t == this._button);
		},
		hide:function(){
			var t = this;
			t._visible = false;
