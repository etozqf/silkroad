 ...
 * @version    $Id: cmstop.treetable.js 7652 2012-11-28 01:19:59Z liyawei $
 */
(function(ct,$){
    ...
    var findChildren = function(tr, expectLevel) {
        var level = parseInt(tr.attr('level') || 0), currentLevel, next, children = [];
        while ((next = tr.next()).length && (currentLevel = parseInt(next.attr('level') || 0)) > level) {
            if (expectLevel === undefined || currentLevel == expectLevel) children.push(next);
            tr = next;
        }
        return children;
    };
    ...
    ct.treeTable = function(table, options)
    {
        ...
        var tbody = $(table[0].tBodies[0]),
            ...
            sortFunc = ct.func(options.sort) || null,
        ...
        var sortRows = function(level, parentTr) {
            var rows = parentTr
                ? findChildren(parentTr, level)
                : tbody.find('[level='+level+']'),
                prev = parentTr;
            if (rows.length < 2) return;
            rows.sort(sortFunc);
            $.each(rows, function() {
                var row = $(this),
                    children = findChildren(row);
                if (prev) {
                    prev.after(row);
                } else {
                    tbody.append(row);
    	...
    	var setParent = function($tr, parent, place)
	    {
    	    ...
    	    if (place && place.jquery) {
    	    	...
    	    	if (parentTr) {
                    if (getInsertAfter(parentTr)[0] == $tr[0]) return;
                    getInsertAfter(parentTr).after($tr);
                } else {
                    tbody.append($tr);
                }
    	    }
            sortFunc && sortRows(level, parentTr);
	    };
    	...
    	var fillin = function(json) {
    		...
    		if ($.isArray(json)) {
    			...
    			for (var i=0,item;item=json[i++];) {
                    // in chrome, numerical property will be sort automatically.
                    // @see http://code.google.com/p/v8/issues/detail?id=6#c7
                    // prefix the numerical key to avoid this behavior.
	    			temp['key-'+item[idField]] = item;
	    		}
    		} else {
    			temp = $.extend({}, json);
    		}
