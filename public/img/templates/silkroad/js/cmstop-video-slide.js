 ...
 */
 (function(scope){
	...
	var init = function(c){
			...
			thumb_links = thumb_box.find('li');
			max_len = thumb_links.size();
	...
	var cacheSpace = function(){
			for(var i = 0 ; i < thumb_links.size();i++){
				if(i == 0){
					thumb_spaces.push(0);
					continue;
				} 
	...
	//绑定事件
	var handle = function(){
			thumb_links.hover(function(){
				thumb_bg.stop();
				clearInterval(timer);
				var i = thumb_links.index($(this));
				if(i == now_i) return;
				run(i);
			},function(){
				autoPlay();
	...
	var updateThumb = function( i ){
		var cls = 'now';
		thumb_links.removeClass(cls).eq(i).addClass(cls);
	}
