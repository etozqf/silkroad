...
_fillData = function(data){
	...
	{
		var d = data[i];
		html[k++] = '<li class="item"><em class="ico"></em><span class="date">'+d.date+'</span><a class="title-link" target="_blank" href="'+d.caturl+'">['+d.catname+']</a><a class="title" target="_blank" href="'+d.url+'">'+d.title+'</a></li>';
		if ((i+1)%5 == 0) {
...
var APP = {
	init:function(channelid,pagesize,interval){
		...
		channels = $('#data-channel>li').each(function(){
			var a = $('a', this);
	...
	channel:function(channel){
		channels.isnot(function(){
			...
			$.className.add(this, 'now');
		},function(){
			$.className.remove(this, 'now');
		});
		_channel = channel;
		this.query();
	},
...
$(function() {
    ...
    picker.DatePicker({
        format: 'yyyy-MM-dd',
        change: function(val) {
            APP.date(val && val.length ? val : picker.attr('data-default'));
        }
    });
});