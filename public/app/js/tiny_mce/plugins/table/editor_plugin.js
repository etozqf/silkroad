					...
					ed.plugins.contextmenu.onContextMenu.add(function(th, m, e) {
						...
						if (ed.dom.getParent(e, 'td') || ed.dom.getParent(e, 'th')) {
							...
							if (el.nodeName == 'IMG' && el.className.indexOf('mceItem') == -1) {
                                ...
                                m.add({title : '处理图片', onclick : function(){
                                    ct.editImage(ed.dom.getAttrib(el, 'src'),function(json){
                                        ed.dom.setAttribs(el,{'src':UPLOAD_URL+json.file+'?'+Math.random()*5,'width':null,'height':null});
                                    });
                                }});
								m.addSeparator();
							}
				...
				case "mceTableRowProps":
					...
					if (user_interface) {
						inst.windowManager.open({
							...
							height : 305 + parseInt(inst.getLang('table.rowprops_delta_height', 0)),
							inline : 1
						}, {
							plugin_url : url
						});
				...
				case "mceTableCellProps":
					...
					if (user_interface) {
						inst.windowManager.open({
							...
							height : 305 + parseInt(inst.getLang('table.cellprops_delta_height', 0)),
							inline : 1
						}, {
							plugin_url : url
						});
				...
				case "mceInsertTable":
					if (user_interface) {
						inst.windowManager.open({
							...
							height : 330 + parseInt(inst.getLang('table.table_delta_height', 0)),
							inline : 1
						}, {
							plugin_url : url,
