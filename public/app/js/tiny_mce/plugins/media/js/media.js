...
var isflv = false, flvurl = '', flvplayer = IMG_URL + 'apps/video/js/flvplayer.swf';	//定义flv所需变量
...
function init() {
	...
	f = document.forms[0] //form对象

	fe = ed.selection.getNode(); //取得选择当前代码的tag对象 
	...
	if (pl != "") {
		...
		switch (type) {
			case "flash":
				/* 增加对FLASH类型的FLV支持判断*/
				if(pl['flashvars']){
					if (/\.flv$/i.test(pl['flashvars']))
					{
						isflv  = true;
						paraString = pl['flashvars'].split("&");
						var paraObj = {};
						for (i=0; j=paraString[i]; i++){  
							paraObj[j.substring(0,j.indexOf("=")).toLowerCase()] = j.substring(j.indexOf("=")+1,j.length);
						}
						flvurl = unescape(paraObj['file']);
						pl['src'] = flvurl;
						pl['flashvars'] = pl['flashvars'].replace('file='+escape(flvurl),'');
					}
				}
				/* flv判断结束 */
...
function insertMedia() {
	...
	if(f.videocode.value) {
		ed.execCommand('mceInsertContent', false, f.videocode.value);
		tinyMCEPopup.close();
		return false
	}
	...
	f.width.value = f.width.value == "" ? 400 : f.width.value;
	f.height.value = f.height.value == "" ? 300 : f.height.value;
...
function getType(v) {
	...
	fo = ed.getParam("media_types", "flash=swf,flv;shockwave=dcr;qt=mov,qt,mpg,mp3,mp4,mpeg;shockwave=dcr;wmp=avi,wmv,wm,asf,asx,wmx,wvx;rmp=rm,ra,ram").split(';');
...
function getStr(p, n, d) {
	...
	/* 增加对FLASH类型的FLV支持判断*/
	if (n == 'flashvars' && isflv)
	{
		v = e.value + (e.value ? '&' : '') + 'file=' + escape(flvurl);
	}
	if (n == 'src' && isflv)
	{
		v = flvplayer;
