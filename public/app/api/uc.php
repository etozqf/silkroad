...
class uc_note {
	...
	function deleteuser($get, $post) {
        ...
        if ($uids) {
            ...
            if ($uids) {
                $this->db->delete("DELETE FROM #table_member WHERE userid IN ($uids)");
            }
        }
	...
	function renameuser($get, $post) {
		...
		$this->db->prepare("UPDATE #table_member SET `username`=? WHERE userid=?")->execute(array(
            $usernamenew, $uid
        ));
	...
	function synlogin($get, $post) {
		...
		if($m) {
			$this->db->prepare("UPDATE #table_member
              SET `lastloginip`=:ip,`lastlogintime`=:time,logintimes=logintimes+1
              WHERE `userid`=:userid")->execute(array(
                'ip' => IP,
                'time' => TIME,
                'userid' => $m['userid']
            ));
			$cookie->set('auth', str_encode($m['userid']."\t".$m['username']."\t".$m['groupid']."\t".md5($_SERVER['HTTP_USER_AGENT'])."\t".IP, config('config', 'authkey')));
			$cookie->set('userid', $m['userid']);
			$cookie->set('username', $m['username']);
			$cookie->set('rememberusername', $m['username']);
		} else {
			$cookie->set('rememberusername', $username, TIME + 2592000);
		}
		return API_RETURN_SUCCEED;
	}
	...
	function updatepw($get, $post) {
		...
		$newpw = md5($password);
		$this->db->prepare("UPDATE #table_member SET password=? WHERE username=?")->execute(array(
            $newpw, $username
        ));
		return API_RETURN_SUCCEED;
	}
	...
	function updatecredit($get, $post) {
		...
		//更新积分
		$this->db->prepare("UPDATE #table_member SET credits=credits+? WHERE userid=$uid")->execute(array(
            $amount, $uid
        ));
