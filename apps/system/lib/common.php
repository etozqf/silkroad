...
function readprivileges($contentid, $userid = 0, $returncharge = 0) {
    ...
    //收费等级， 1、文章收费；2、栏目收费；3、属性收费；文章收费优先级最大。 属性收费优先级最低。 如果栏目父级是收费的，则自己也是收费的。 属性只能是本身是收费的，才能收费，不继承父类属性。
    $chargelevel = 0;
    $ischarge = 0;
    if($content['ischarge']) {
        $chargelevel = 1;
        ...
        if($category) {
            ...
            if($category['ischarge']) {
                ...
                $exists = $db->select("SELECT catid FROM #table_category WHERE catid IN (".$category['parentids'].") AND ischarge=1 LIMIT 1");
                if($exists) {
                    $chargelevel = 2;
                }
            }
            $category['parentid'] && $catids = array_merge($catids, explode(',', $category['parentids']));
        }

        if(!$chargelevel) {
            $propertys = array();
            $data = $db->select("SELECT * FROM #table_content_property AS cp LEFT JOIN #table_property AS p ON cp.proid=p.proid WHERE cp.contentid=$contentid");
            $proids = array();
            if($data) {
                foreach($data as $property) {
                    if($property['ischarge']) {//自己是收费的
                        $chargelevel = 3;
                        $propertys[$property['proid']] = $property;
                    }
                }
            }
        }

        $baogao_config = config::get('baogao');
        if($chargelevel == 3) {//属性收费时，判断这些属性（属性可能会是多个）中是否包含报告中的属性，如果有，则需要进一步判断此栏目是否是报告栏目，如果不是则免费
            $baogao_proids = $baogao_proids_en = array(); //所有的报告属性
            if($baogao_config['proid']) {
                $baogao_proids[] = $baogao_config['proid'];
                $baogao_property = $db->get("SELECT * FROM #table_property WHERE proid='$baogao_config[proid]'");
                $baogao_property['childids'] && $baogao_proids = array_merge($baogao_proids, explode(',', $baogao_property['childids']));
            }

            if($baogao_config['proid_en']) {
                $baogao_proids_en[] = $baogao_config['proid_en'];
                $baogao_property_en = $db->get("SELECT * FROM #table_property WHERE proid='$baogao_config[proid_en]'");
                $baogao_property_en['childids'] && $baogao_property_en = array_merge($baogao_proids_en, explode(',', $baogao_property_en['childids']));
            }

            $proids = array_keys($propertys);
            $proids_china = array_intersect($proids, $baogao_proids); //中国属性
            $proids_en = array_intersect($proids, $baogao_property_en); //海外属性


            //当是中国属性或海外属性，判断当前栏目是否是报告栏目，如果不是，则免费
            if(($proids_china || $proids_en) && !(in_array($baogao_config['catid'], $catids) || in_array($baogao_config['catid_en'], $catids))) {
                $chargelevel = 0;
            }
        }
    }
    ...
    //管理员不收费
    if($member['groupid'] == 1 || $member['isfree']) {
        return true;
    }
    //判断用户是否有权限
    ...
    if($chargelevel) {
        ...
        if($read) return true;
        if($chargelevel == 2) {
            $read = $db->select("SELECT * FROM #table_member_readmanage WHERE userid=$userid AND idtype='catid' AND id IN (".implode(',',$catids).")");
            if($read) return true;
        } elseif($chargelevel == 3) {

            if($proids_china) {
                $read = $db->select("SELECT * FROM #table_member_readmanage WHERE userid=$userid AND idtype='proid' AND id IN (".implode(',',$proids_china).")");
            } else {
                $proids = array();
                foreach($propertys as $value) {
                    $proids[] = $value['proid'];
                    $value['parentid'] && $proids = array_merge($proids, explode(',', $value['parentids']));
                }
                $read = $db->select("SELECT * FROM #table_member_readmanage WHERE userid=$userid AND idtype='proid' AND id IN (".implode(',',$proids).")");
            }
