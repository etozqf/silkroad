...
<?php
$js = array('article', 'picture', 'video', 'interview', 'activity', 'vote', 'survey', 'special','item');
foreach ($js as $r)
{
...
?>
    ...
    <div id="ischarge">
		<dl class="proids">
			<dt>是否收费 ：</dt>
			<dd>
				<a href="javascript:;" data-value="all" class="checked">全部</a>
				<a href="javascript:;" data-value="1" class="">是</a>
				<a href="javascript:;" data-value="0" class="">否</a>
			</dd>
		</dl>
	</div>
<?php
...
var html = {
	modelmap : {'article':'文章','picture':'组图','video':'视频','activity':'活动','vote':'投票','interview':'访谈','survey':'调查','item':'项目'},
	loadingBox : null,
	maxlimit : null,
	where : null,
	limit : 0,
...
$('#ischarge a').click(function(){
	if($(this).attr("class") != 'checked')
	{
		$(this).siblings(".checked").removeClass("checked");
		$(this).addClass("checked");
		ischarge = 'all';
		ischarge = $(this).attr('data-value');
		tableApp.load("ischarge="+ischarge);
	}
});
...
function pushToFree(contentid,modelid) {
	
	// alert(modelid);return false;
	if(modelid==1){
		$.getJSON('?app=system&controller=handpush&action=get_contentid_article', {contentid:contentid}, function(json) {
          ...
          if (json && json.state) {
 			...
 			$.getJSON('?app=system&controller=push_videohand&action=get_contentid_video', {contentid:contentid}, function(json) {
          ...
          if (json && json.state) {
 			...
 			$.getJSON('?app=system&controller=push_picturehand&action=get_contentid_picture', {contentid:contentid}, function(json) {
