<form name="add" id="category_add" method="POST" action="<?=$form_action?>">
    ...
    <table border="0" cellspacing="0" cellpadding="0" class="table_form yyss">
        ...
        <tr>
            <th>栏目拼音：</th>
            <td><input type="text" name="pinyin" id="pinyin" value="" size="30"/></td>
        </tr>
        <tr>
            <th>栏目缩写：</th>
            <td><input type="text" name="abbr" id="abbr" value="" size="30"/></td>
        </tr>
		<tr>
			<th>是否收費：</th>
			<td>
        ...
        <tr>
            <th>列表页每页信息数：</th>
            <td>&nbsp;<input type="text" name="pagesize" value="<?php echo $pagesize;?>" size="5"/> <span class="c_gray"> 每个栏目可独立设置，留空或为 0 则取 <a href="javascript:;" onclick="ct.assoc && ct.assoc.open('?app=system&controller=setting&action=optimize', '_blank');">系统设置</a> 中的值</span></td>
        </tr>
        <tr>
            <th>允许用户投稿：</th>
        ...
        <tr>
            ...
            <td>
                <select name="watermark">
                    ...
                    <option value="<?php echo $item['watermarkid'];?>"><?php echo $item['name'];?></option>
                    <?php endforeach;?>
                </select>
            </td>
        </tr>
