<tr>
	...
	<td>
		<input type="text" name="author" class="suggest"
			width="300"
			url="?app=system&controller=template&action=suggestDiscuzUser&dsnid=<?=$dsnid?>&prefix=<?=urlencode($prefix)?>&version=<?=$version?>&keyword=%s"
			allUrl="?app=system&controller=template&action=pageDiscuzUser&dsnid=<?=$dsnid?>&prefix=<?=urlencode($prefix)?>&version=<?=$version?>"
			paramPage="page" />
	</td>
</tr>
<tr>
