...
var func = {
	edit:function(id,tr,json){
		ct.form('编辑'+title_table[json.type],'?app=system&controller=openaca&action=edit&acaid='+id,
		380,180,
		function(json){
			if (json.state)
			{
	...
	addApp:function(){
		ct.form('添加应用','?app=system&controller=openaca&action=add',
		300,180,addrow);
	},
	addController:function(id,tr,json){
		ct.form('添加控制器','?app=system&controller=openaca&action=add&parentid='+id,
		300,180,addrow);
	},
	addAction:function(id,tr,json){
		ct.form('添加动作','?app=system&controller=openaca&action=add&parentid='+id,
		380,180,addrow);
	},
	del:function(id,tr){
		ct.confirm('此操作不可恢复，且级联删除，确定要删除吗？',function(){
			$.getJSON('?app=system&controller=openaca&action=delete&acaid='+id, function(json){
				if (json.state) {
					tree.deleteRow(id);
...
var tree = new ct.treeTable('#treeTable',{
	...
	collapsed:true,
	baseUrl:'?app=system&controller=openaca&action=tree',
