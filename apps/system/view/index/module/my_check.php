...
<ul class="txt_list">
<?php 
$data = loader::model('admin/content')->ls(array('status' => 3, 'iscontribute' => 0), '*', 'c.contentid DESC', 0, 10);
if($data)
{  
   foreach ($data as $r)
   {
   	   ...
   	   $r['catname'] = $cat['name'];
?>
   <li><span class="f_r date"><?=$r['created']?></span><span class="f_l"><a href="<?=$r['caturl']?>">[<?=$r['catname']?>]</a> <a href="javascript: ct.assoc.open('<?=content_url($r['contentid'], $r['modelid'])?>', 'newtab');"><?php echo str_natcut($r['title'],30); ?></a></span></li>
<?php 
   }
}
?>
