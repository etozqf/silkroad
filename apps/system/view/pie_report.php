<?php echo '<?xml version="1.0" encoding="'.$CONFIG['charset'].'"?>';?>
<!-- bocolor为背景色（十六进制），bgimg为背景图片地址（没用就用none） -->
<<?php echo $report_name;?> bgcolor="0xFFFFFF" bgimg="none">
<?php foreach ($data as $data): ?>
	<pie>
		<!-- pie名：字符串 -->
		<pie_mcname><?php echo $report_name;?></pie_mcname>
		<!-- pie深度：整数 -->
		<pie_depth>2</pie_depth>
		<!-- 标题：字符串。格式：标题(字符串),x坐标(数字),y坐标(数字),文字大小(数字),是否粗体(true是/false否),颜色（十六进制） -->
		<pie_title><?php echo $data['subject'];?>,210,40,16,true,0x000000</pie_title>
		<!-- 竖向列标支持动作:onRelease,onRollOver,onRollOut,none，任选或除none之外的全部/部份用“,”隔开 -->
		<pie_action>onRelease</pie_action>
		<!-- 数值：数字。每一个项目数据用“,”隔开 -->
		<pie_num><?php echo $data['votes'];?></pie_num>
		<!-- 名称：字符串。每一个项目名称用“,”隔开 -->
		<pie_name><?php echo $data['options'];?></pie_name>
		<!-- 颜色：字符串（十六进制颜色名）。每一个项目颜色用“,”隔开 -->
		<pie_color><?php echo $data['colors'];?></pie_color>
		<!-- 是否显示名称与数值：true显示/false不显示。每一个项目(饼上)是否显示用“,”隔开 -->
		<pie_showtitle><?php echo $data['showtitle'];?></pie_showtitle>
		<!-- 默认pie位置是否打开：true打开/false不打开。默认当前一个项目是否为打开状态用“,”隔开 -->
		<pie_defaultsate><?php echo $data['defaultsate'];?></pie_defaultsate>
		...
		<pie_percentshow>num</pie_percentshow>
	</pie>
<?php endforeach; ?>
