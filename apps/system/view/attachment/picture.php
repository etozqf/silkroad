...
<div class="pop_box_area" id="window_all">
  <?php if($_GET['select']):?>
	...
	<div class="tag_1" style="margin-top:5px;">
		<ul class="tag_list">
			<li id="local_tab"><span><a href="?app=picture&controller=picture&action=image<?php if($single): ?>&single=1<?php endif; ?>" onmousedown="return false;">本地上传</a></span></li>
  ...
  <div class="attachment_lib" id="window_main">
    ...
    <div class="box_10 attachment_r" id="window_right">
      ...
      <div class="lh_32 t_c" style="float:right; margin-top:2px;">
        <input type="button" value="确定" onclick="insert()" class="button_style_1" style="float:left;width:84px;"/>
        <input type="button" value="取消" onclick="window.dialogCallback.close();" class="button_style_1" style="float:left;width:84px;"/>
      </div>
    </div>
    <div class="clear"></div>
  </div>
...
var insert = function() {
	var li = $("li.selected"),
        aid, src, note, total = li.length, now = 0,
        item, delayAdd;
    if (! total) {
        ct.error('请选择要添加的图片');
        return false;
    ...
    delayAdd = function() {
        item = li.eq(now++);
        aid = item.attr('id').substr(5);
		src = item.attr('src');
        note = item.find('[class=filename]').text();
        note = note && note.indexOf('.') > -1 ? (note.substr(0, note.lastIndexOf('.'))) : note;
		if (window.dialogCallback.insert(aid, src, note, 0, now < total) !== false) {
            file_click(item.get(0));
            if (now < total) {
                setTimeout(delayAdd, 50);
            } else {
                ct.endLoading();
    ...
    ct.endLoading();
    ct.stopListenOnce();
    ct.startLoading('center', '正在添加选中的 ' + total + ' 张图片...');
	delayAdd();
}
</script>
<div id="notice" ></div>
