...
function ok(){
	$.post('?app=system&controller=score&action=index',{userid:$('input[name="createdby"]:hidden').val(),score:$('input[name="score"]:hidden').val(),comment:$('textarea[name="comment"]').val(),contentid:<?=$contentid?>},function(){
