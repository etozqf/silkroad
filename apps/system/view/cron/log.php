...
<div class="table_foot">
	...
	<?php else: ?>
        ...
        <select id="order" name="order">
            <option value="logid">按ID排序</option>
            <option value="expend">按执行时间排序</option>
        </select>
	<?php endif; ?>
...
var tpl = '<tr id="row1_{logid}">\
                    ...
                    <td class="t_c">{fintime}</td>\
                	<td class="t_c">{runSuccess}</td>\
                	<td class="t_c tipClass" tips="{info}">{displayInfo}</td>\
                </tr>';
...
$(function (){
    ...
    $('#order').bind('change', function() {
        var order = $(this).children(':selected');
        logTable.load('order='+order.attr('value'));
    })
})
