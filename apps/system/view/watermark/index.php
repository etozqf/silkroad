...
var defaultWatermark = <?php echo empty($default_watermark)?'""':$default_watermark;?>;
...
var wm = {
	'add' : function() {
		ct.formDialog('添加水印方案', '?app=<?php echo $app;?>&controller=<?php echo $controller;?>&action=add', function(json) {
			if (json.state) {
				tableApp.reload();
				return true;
			} else {
	...
	'edit' : function(id) {
		ct.formDialog('编辑水印方案', '?app=<?php echo $app;?>&controller=<?php echo $controller;?>&action=edit&id='+id, function(json) {
			if (json.state) {
				tableApp.reload();
				return true;
			} else {
	...
	'setDefault' : function(id) {
		$.getJSON('?app=<?=$app?>&controller=<?php echo $controller;?>&action=set_default&id='+id, {}, function(json) {
			if (json.state) {
				defaultWatermark = id;
				tableApp.reload();
				return true;
	...
	'unsetDefault' : function() {
		$.getJSON('?app=<?php echo $app;?>&controller=<?php echo $controller;?>&action=unset_default', {}, function(json) {
			if (json.state) {
				defaultWatermark = '';
				tableApp.reload();
				return true;
	...
	'disable' : function(id) {
		$.getJSON('?app=<?php echo $app;?>&controller=<?php echo $controller;?>&action=disable&id='+id, {}, function(json) {
			if (json.state) {
				tableApp.reload();
				return true;
			}
	...
	'delete' : function(id) {
		$.getJSON('?app=<?php echo $app;?>&controller=<?php echo $controller;?>&action=delete&id='+id, {}, function(json) {
			if (json.state) {
				tableApp.reload();
				return true;
			}
	...
	'rowCallback' : function(id,e) {
		if (id == defaultWatermark) {
			...
			e.find('.setdefault').find('a').eq(0).html('取消默认').bind('click', function() {
				wm.unsetDefault();
			});
		}
...
var tableApp = new ct.table('#item_list', {
	...
	'baseUrl'		: '?app=<?php echo $app;?>&controller=<?php echo $controller;?>&action=page'
});
$(document).ready(function() {
	tableApp.load();
	$.validate.setConfigs({
		xmlPath:'apps/system/validators/watermark/'
	});
});
</script>
