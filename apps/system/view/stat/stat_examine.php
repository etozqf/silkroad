...
<table  border="0" width="100%" border="0" cellpadding="0" cellspacing="0">
  ...
  <tr>
    <td width="320">
		<input class="input_calendar" name="mintime" value="<?php echo date('Y-m-1');?>" /> ~
		<input class="input_calendar" name="maxtime" value="<?php echo date('Y-m-d');?>" />
	</td>
    <td width="72"><select id="model" name="modelid" style="width:80px;" marginTop="1" marginLeft="0" multiple="multiple">
...
var tableApp = new ct.table('#item_list', {
	...
	jsonLoaded : function(json){
		...
		$.each(json.data, function(i,item) {
			statInfo.posts		+= parseInt(item.posts) || 0;
			statInfo.pv			+= parseInt(item.pv) || 0;
			statInfo.comments	+= parseInt(item.comments) || 0;
			statInfo.score		+= parseInt(item.score) || 0;
		});
		$('#item_list').find('td.posts').html(statInfo.posts);
		$('#item_list').find('td.pv').html(statInfo.pv);
		$('#item_list').find('td.comments').html(statInfo.comments);
		$('#item_list').find('td.score').html(statInfo.score);
	},
...
$(function(){
	$('.input_calendar').DatePicker({'format':'yyyy-MM-dd'});
});
