...
*/
class controller_admin_zixun2017 extends system_controller_abstract{
	...
	public function __construct($app)
	{
		...
		$this->api_url="?app=article&controller=article&action=zx_add";
		$this->db=factory::db();
		$this->config=config("2017push",'zx');	//找到配置文件中,资讯推送对应规则
		$this->pagesize=200;
...
function getCategoryList($catid,$free_catid)
{
	$pagesize=200;	//每次查询数量
	/*记录总条数*/
	$total=$this->db->select("select count(contentid) as count from #table_content where catid=$catid and markid=0 and status=6 and published>".$this->start_time." and published <".$this->end_time);
	...
	for($i=1;$i<=$count;$i++)
	{
		...
		$list=$this->db->select("select c.*,a.* from #table_content as c inner join #table_article as a on c.contentid=a.contentid and catid=$catid and markid=0 and status=6 and published > ".$this->start_time." and published <".$this->end_time." $limit");
