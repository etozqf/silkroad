...
class controller_api_article extends api_controller_abstract
{
    ...
    */
    ...
    function getvideo()
    {
        ...
        $db = factory::db();
        $start = ($page-1)*$pagesize;
        $sql = "SELECT c.*,v.* FROM cmstop_content AS c LEFT JOIN cmstop_video AS v ON c.contentid=v.contentid WHERE c.modelid=4 AND c.status=6 ORDER BY c.published DESC LIMIT $start,$pagesize";
        $sqlcount = "SELECT count(c.contentid) AS count FROM cmstop_content AS c LEFT JOIN cmstop_video AS v ON c.contentid=v.contentid WHERE c.modelid=11 AND c.status=6";
        $total = $db->select($sqlcount);
        $totalpage=ceil($total[0]['count']/$pagesize);
        $data = $db->select($sql);
        if(!empty($data))
