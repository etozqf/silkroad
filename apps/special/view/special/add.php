...
<table width="98%" border="0" cellspacing="0" cellpadding="0" class="table_form mar_l_8">
	...
	<tr id="theFolder">
		<th>目录名称：</th>
		<td><input id="folder" value="<?=$folder?>" data-path="<?=$path?>" /> <a href="javascript:;" onclick="$('#theFolder').hide().next().show()">自定义</a></td>
	</tr>
	<tr style="display: none;">
		<th><span class="c_red">*</span> 网址：</th>
		<td>
			<?php echo element::psn('path', 'path', $path.'/'.$folder);?>
		</td>
	</tr>
    <tr>
	...
	<tr>
		...
		<td><input type="text" name="lastupdated" class="input_calendar" value="" size="20"/></td>
	</tr>
</table>
...
// 获取自定义字段
$(function() {
    var p = $('#path'), fd = $('#folder').input(function(){
    	p.val(fd.attr('data-path')+'/'+fd.val());
    });
    fd.input();
    var catid = $('#catid');
