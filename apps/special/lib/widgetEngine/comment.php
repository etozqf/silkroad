...
class widgetEngine_comment extends widgetEngine
{
	...
	public function _render($widget)
	{
		...
		$r = array(
			'commentUrl' => APP_URL.url('comment/review/add'),
	...
	public function _addView()
	{
		...
		$this->view->assign('topicid', $this->_getTopicId());
    ...
    protected function _getTopicId()
    {
        ...
        {
            $db = factory::db();
            $r = $db->get("SELECT `contentid` FROM `#table_special_page` where pageid='$pageid'");
