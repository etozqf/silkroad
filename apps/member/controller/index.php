...
class controller_index extends member_controller_abstract
{
	...
	function login()
	{
		...
		if ($this->is_post())
		{
			...
			$member = $this->member->login($_POST['username'], $_POST['password'], 1800);
			if (!$member)
			{
	...
	static function make_str()
  {
	...
	*/
	...
	{	
			...
			$data['email'] = trim($_POST['emailaddress']);
		    $data['invitationCode']=intval(trim($_POST['invitationCode']));	/*邀请码*/
...
/*        if($this->verificationPathswitch)
	...
	private function _check_register_newcn()
	{
		...
		/*验证邀请码是否正确*/
		if(!$this->check_invitation_code_cn($_POST['emailaddress'],$_POST['invitationCode']))
		{
			$this->error='Invite code error, reenter the correct invitation code or reapply';
			return false;
		}
	...
	/*英文站检测邀请码*/
	public function check_invitation_code_cn($email,$code)
	{
	
		if(!isset($_SESSION[$email]['invitationCode'])){
			$this->error="The invitation code has expired. Please re apply the invitation code.";
			return false;
		}

		if(intval($_SESSION[$email]['invitationCode'])!=$code){
			$this->error="Invite code error, re enter the correct invitation code or reapply";
			return false;
		}else{
			/*邀请码正确后，清除该Session*/
			unset($_SESSION[$email]['invitationCode']);
			return true;
		}
