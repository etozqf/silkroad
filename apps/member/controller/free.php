...
*/
class controller_free extends member_controller_abstract
{
	...
	function login()
	{
		...
		if ($this->is_post())
		{
			...
			$arr=[];
			$arr["auth"]=$_COOKIE[$prefix."auth"];
			$arr["username"]=$_COOKIE[$prefix."username"];
			$cc=json_encode($arr);	

			$redirect="http://app.silkroad.news.cn/?app=system&controller=freelogin&action=ceshi&list=".$cc;
	...
	static function make_str()
  {
	...
	*/
	...
	function logout()
	{
		...
		$url="http://app.silkroad.news.cn/?app=system&controller=freelogin&action=free_exit&list=1";
		$this->redirect($url);
	}
