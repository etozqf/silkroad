...
class model_member_front extends model implements SplSubject
{
	...
	function __construct()
	{
		...
		$this->_fields = array('userid', 'username', 'password', 'email', 'groupid', 'avatar', 'regip', 'regtime', 'lastloginip', 'lastlogintime', 'logintimes', 'posts', 'comments', 'pageviews', 'credits','salt','amount', 'status','site_type','online_code', 'max_online', 'now_online', 'login_type', 'first_online', 'now_online_times', 'isfree', 'thirdtype', 'thirdid', 'thirdtoken', 'thirdlastlogin', 'login_ip','lifetime','mark');
	...
	*/
	...
	function register_cn($data)
	{
		...
		else
		{
            ...
            $data['salt'] = self::make_salt();
			$data['password'] = self::make_password($data['password'], $data['salt']);
			$data['groupid']=20;	//英文站试用用户。
			$data['lifetime']=strtotime('+24 hour');	//账号有效期在24小时之内
			$data['mark']=1;	//值为1时代表从英文站前台注册,默认值为0
			$userid = $this->insert($data);
			if ($userid)
			{
				$data['userid'] = $userid;
                $data['mobile'] = $data['mobile'];
                $data['mobileauth'] = $data['mobile'] ? 1 : 0;
                $a = $this->member_detail->add($data);
                //修改detail表
                if($a){
                	$data1 = $this->filter_array($this->data, array('name', 'sex', 'birthday', 'telephone', 'mobile', 'job', 'address', 'zipcode', 'qq', 'msn', 'authstr','remarks', 'mobileauth','country'));
                	$sql = "update cmstop_member_detail set name='$data1[name]',sex='$data1[sex]',telephone='$data1[telephone]',job='$data1[job]',address='$data1[address]',remarks='$data1[remarks]',country='$data1[country]' where userid=$userid";
                	$db = & factory::db();
                	$db->select($sql);
	...
	{
		$this->m = $this->get_by('username', $username, 'userid,username,password,groupid,salt,status,site_type,max_online,login_type,now_online');
		$this->error = null;
		$this->ucsynlogin = '';
		$this->username = $username;
		$this->password = $password;
		...
		$cookie->set('userip', IP, $cookietime);								  
	...
	function login_newcn($username, $password, $cookietime = 0, $without_password = false)
	{
		$this->m = $this->get_by('username', $username, 'userid,username,password,groupid,salt,status,site_type');
		$this->error = null;
		$this->ucsynlogin = '';
	...
	function logout($logintime,$userid)
	{
		$cookie = factory::cookie();
		$db = factory::db();
		...
		if($now_online>1){
			//修改now_online_times字段,删除这次登录
			$now_online_times = json_decode(table('member',$userid,'now_online_times'));
			//echo '<pre>';print_r($now_online_times);
			foreach($now_online_times as $k=>$v){
				if($v==$logintime){
					unset($now_online_times[$k]);
				}
			}
			//修改firsr_online字段
			sort($now_online_times);
			$first_online = $now_online_times[0];
			$now_online_time = json_encode($now_online_times);
			$sql = "UPDATE cmstop_member SET now_online=$now_online-1,first_online=$first_online,now_online_times='$now_online_time' WHERE userid=$userid";
			$db->select($sql);
			

		}elseif($now_online==1){
			//全部清0
			$sql = "UPDATE cmstop_member SET now_online=0,first_online=0,now_online_times='' WHERE userid=$userid";
			$db->select($sql);
		}
		...
		$cookie->set('password');
		$cookie->set('groupid');
		$cookie->set('site_type');
		$cookie->set('photo');
		//$cookie->set("$username".'_now_online');
		//$cookie->set("$username");
