...
<table border="0" cellspacing="0" cellpadding="0" class="table_form">
	<tbody id="profilebasic">
		...
		<th>所属站点：</th>
			<td>
				<label><input type="radio" name="site_type" value="1" <?php if($site_type==1) echo 'checked="checked"'; ?>/>中文站</label>
				&nbsp;
				<label><input type="radio" name="site_type" value="2" <?php if($site_type==2) echo 'checked="checked"'; ?>/>英文站</label>
			</td>
		</tr>
		<tr>
			<th>用户组：</th>
		...
		<tr>
			<th>全站免费：</th>
			<td>
			<label><input type="radio" name="isfree" value="1" <?php if($isfree) echo 'checked="checked"'; ?>/>是</label>
			&nbsp;
			<label><input type="radio" name="isfree" value="0" <?php if(!$isfree) echo 'checked="checked"'; ?>/>否</label>
			</td>
		</tr>
		<tr>
			<th>绑定分站：</th>
			...
			<select name="thirdtype">
				...
				<option value="1" <?php if($thirdtype == 1) echo 'selected="selected"'; ?>>广西分站</option>
			</select>
			</td>
		</tr>
		<tr>
			<th>分站ID：</th>
			<td>
				<input type="text" name="thirdid" id="thirdid" value="<?php echo $thirdid ? $thirdid : ''; ?>" />
			</td>
		</tr>
		...
		<tr>
			<th>性别：</th>
			<td>
			<input class="radio radio_style" type="radio" name="sex" value="1" <?php if($sex == 1):?> checked<?php endif;?> /> 男 
			<input class="radio radio_style" type="radio" name="sex" value="2" <?php if($sex == 2):?> checked<?php endif;?> /> 女 </td>
		</tr>
		<tr>
			<th>生日：</th>
			<td>
		...
		<tr>
			<th>状态：</th>
			<td>
				<label><input type="radio" name="status" value="0"<?php if($status==0):?> checked="checked"<?php endif;?> />禁用</label>
				&nbsp;
				<label><input type="radio" name="status" value="1"<?php if($status==1):?> checked="checked"<?php endif;?> />启用</label>
			</td>
		</tr>
		<tr>
			<th>账号有效期：</th>
			<td>
				<input type="text" name="lifetime" id="lifetime" class="input_calendar" size="20" value="<?php echo date("Y-m-d H:i:s",$lifetime);?>"/>
			</td>
		</tr>
		<tr>
			<th>最大同时在线人数：</th>
			<td><input type="text" name="max_online" id="max_online" value="<?=$max_online?>" size="10" /></td>
		</tr>
		<tr style="display:none">
			<th>登录类型：</th>
			<td><input type="text" name="login_type" id="login_type" value="<?=$login_type?>" size="10" /></td>
		</tr>
		<tr>
			<th>ip池限制：<br><span style="color: rgb(153, 153, 153); ">(多ip用;分开)</span></th>
			<td><textarea name="login_ip" id="login_ip" size="10"><?=$login_ip?></textarea></td>
		</tr>
	</tbody>
</table>
</form>
