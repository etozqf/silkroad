...
?>
	...
	<tr>
		...
		<td>
		MySQL 方式
		<input class="radio radio_style" type="hidden" name="uc_connect" id="mysql" value="mysql">
		</td>
	</tr>
	<tbody id="db_setting">
	<tr>
    ...
    <tr>
        <th>数据库端口号：</th>
        <td><input type="text" name="uc_dbport" value="<?=$uc_dbport?>" id="uc_dbport" size="30"></td>
    </tr>
	<tr>
		<th>数据库用户名：</th>
...
function uc_merge(data) {
	if(data.state) {
		if(data.finished == 0) {
			...
			ct.endLoading();
			ct.tips('数据同步完成，请重新登录系统来完成同步', 'success', 'center', 3);
            setTimeout(function() {
                ct.setCookie(COOKIE_PRE + 'auth', null, {
                    path: COOKIE_PATH,
                    domain: COOKIE_DOMAIN
                });
                (window.parent || window).location.href = '<?=ADMIN_URL?>';
            }, 3000);
		}
	} else {
		ct.error(data.error);
