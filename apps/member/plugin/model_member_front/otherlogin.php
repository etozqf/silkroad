...
class plugin_otherlogin extends controller
{
	...
	public function bindPage()
	{
		...
		$auth = $member_api->select(array('userid'=>value(online(),'userid')));
		$thirdlogin_data = $api->page('islogin=1 AND state>0', '*', 'sort');
		$auth_api = array();
		foreach ($auth as $item)
		{
			$auth_api[$item['apiid']] = $item;
		}
		
		foreach ($thirdlogin_data as $key => $item)
		{
			if ($thirdlogin_data[$key]['has_auth'] = !empty($auth_api[$item['apiid']]))
			{
				$thirdlogin_data[$key]['expires_in'] = $auth_api[$item['apiid']]['expires_in'];
			}
		}
		$this->template->assign('_userid', $this->member->_userid);
		$this->template->assign('_username', $this->member->_username);			
...
class member_bind extends model
{
	...
	public function xsyn($m, $v=null)
	{
		...
		$url = 'http://t.db.silkroad.news.cn/index.php?m=account.ctsyn';
