...
<div class="tag_1 mar_l_8">
	<ul class="tag_list">
		<li>
			<a href="?app=comment&controller=comment&action=index&status=2<?php if (!empty($_GET['rwkeyword'])):?>&rwkeyword=<?php echo $_GET['rwkeyword'];?><?php endif;?>" <?= $status == 2 ? 'class="s_3"' : ''?> >已审</a>
		</li>
		<li>
			<a href="?app=comment&controller=comment&action=index&status=1<?php if (!empty($_GET['rwkeyword'])):?>&rwkeyword=<?php echo $_GET['rwkeyword'];?><?php endif;?>" <?= $status == 1 ? 'class="s_3"' : ''?> >待审</a>
		</li>
	</ul>
</div>
...
<div class="tag_list_1 pad_8 layout mar_l_8" id="bytime_list">
	...
	<a href="javascript: tableApp.load('published=<?= date('Y-m-d', date::this_week(true))?>');">本周</a>
	<a href="javascript: tableApp.load('published=<?= date('Y-m-01', strtotime('this month'))?>');">本月</a>
	<div class="clear"></div>
</div>
...
<ul class="f_l">\
	<li>\
		<span class="f_r">\
			<a href="javascript:void(0);" class="u username" onclick="comment.url({createdby})">{username}</a> \
...
function init_row_event(id, tr) {
	...
	/* source parser */
	if (parseInt(json.sourceid, 10) > 0) {
		var username = tr.find('.username');
		username.removeAttr('onclick')
		.html(json.sourceinfo.nick||'')
		.addClass('f_l');
		if (json.sourceinfo.weibo_url) {
			username.attr('href', json.sourceinfo.weibo_url)
			.attr('target', '_blank');
		}
		if (json.sourceinfo.symbol && json.sourceinfo.symbol.ico) {
			var symbol = $('<a style="padding: 0 3px;" class="f_l" href="'+(json.sourceinfo.symbol.url||'javascript:;')+'" target="'+(json.sourceinfo.symbol.url ? '_blank':'_self')+'"></a>').insertAfter(username);
			symbol.append('<img src="'+json.sourceinfo.symbol.ico+'" alt="'+json.sourceinfo.symbol['identity']+'" title="'+json.sourceinfo.symbol['name']+'" />');
		}
	}
}
