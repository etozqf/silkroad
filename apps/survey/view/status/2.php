...
if ($workflowid){
	...
	?>
	      <input type="button" name="pass" onclick="survey.pass();" value="送审" class="button_style_1"/>
	<?php 
	}
...
elseif (priv::aca('survey', 'survey', 'approve')) 
{
?>
    <input type="button" name="approve" onclick="survey.approve();" value="送审" class="button_style_1"/>
<?php 
}
...
if ($workflowid){
	if (priv::aca('survey', 'survey', 'pass')){
	?>
	    <li class="pass"><a href="#survey.pass">送审</a></li>
	<?php 
	}
	if (priv::aca('survey', 'survey', 'reject')){
	?>
...
{
?>
    <li class="approve"><a href="#survey.approve">送审</a></li>
<?php 
}
if (priv::aca('survey', 'survey', 'move')){
?>
...
var row_template = '<tr id="row_{contentid}">';
	...
	row_template +='	<td><a href="javascript:survey.view({contentid})" style="color:{color}" tips="ID：{contentid}<br />Tags：{tags}<br />创建：{createdbyname}（{created}）<br />修改：{modifiedbyname}（{modified}）<br/>题目数：{questions} <br/>人数上限：{maxanswers}" class="title_list">{title}</a> {note} {lock}</td>';
	row_template +='	<td class="t_c">'+action+'</td>';
<?php if ($workflowid && $_roleid < 3){?>
	row_template +='	<td class="t_c">{workflow_rolename}</td>';
<?php }?>
