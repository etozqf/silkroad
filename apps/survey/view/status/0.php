...
?>
      <input type="button" name="restore" onclick="survey.restore();" value="还原" class="button_style_1"/>
<?php 
}
if (priv::aca('survey', 'survey', 'restores')){
?>
      <input type="button" name="restore" onclick="survey.restores(<?=$catid?>);" value="全部还原" class="button_style_1"/>
<?php 
}
if (priv::aca('survey', 'survey', 'delete')){
?>
      <input type="button" name="delete" onclick="survey.del();" value="彻底删除" class="button_style_1"/>
<?php 
}
if (priv::aca('survey', 'survey', 'clear')){
?>
      <input type="button" name="clear" onclick="survey.clear(<?=$catid?>);" value="清空回收站" class="button_style_1"/>
<?php 
}
?>
    </p>
...
if (priv::aca('survey', 'survey', 'delete')){
?>
    <li class="del"><a href="#survey.del">删除</a></li>
<?php 
}
if (priv::aca('survey', 'survey', 'restore')){
?>
    <li class="restore"><a href="#survey.restore">还原</a></li>
<?php 
}
...
var row_template = '<tr id="row_{contentid}">';
	...
	row_template +='	<td><a href="javascript:survey.view({contentid})" style="color:{color}" tips="ID：{contentid}<br /><br />Tags：{tags}<br />创建：{createdbyname}（{created}）<br />修改：{modifiedbyname}（{modified}）<br/>题目数：{questions} <br/>人数上限：{maxanswers}" class="title_list">{title}</a> {note} {lock}</td>';
