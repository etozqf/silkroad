...
class model_search extends model implements SplSubject
{
	...
	public function page($q = array(), $type = 'all', $mode = 'SPH_MATCH_PHRASE', $order = 'TIME_SEGMENTS', $asc = 'DESC', $page, $pagesize)
	{
		$this->keywords = $q['wd'];
	...
	function setSortMode($orderby = 'TIME_SEGMENTS', $asc = 'DESC')
	{
        if($orderby == 'TIME_SEGMENTS')
        {
            $this->cl->SetSortMode(SPH_SORT_TIME_SEGMENTS, 'published');
        }
		elseif($orderby == 'REL')
		{
