...
<div class="tag_list_1 pad_8 layout mar_l_8" id="bytime_list">
	...
	<a href="javascript:log.search('publish_d=<?=date('Y-m-d', date::this_week(true))?>');">本周</a>
	<a href="javascript:log.search('publish_d=<?=date('Y-m-01', strtotime('this month'))?>');">本月</a>
	<div class="clear"></div>
</div>
...
<script type="text/javascript">
	var row_template  ='<tr id="row_{logid}">';
        ...
        row_template +='	<td class="t_c">{role}</td>';
		row_template +='	<td class="t_c">{ip} {location}</td>';
        row_template +='	<td class="t_c">{regip} {location_regip}</td>';
        row_template +='	<td class="t_c">{email}</td>';
