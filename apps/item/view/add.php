...
<form name="item_add" id="item_add" method="POST" action="?app=<?=$app?>&controller=<?=$app?>&action=add">
    ...
    <caption>项目基本情况(以下带*全部为必填项)</caption>
		...
		<tr>
			...
			<td>
                <input type="text" id="countryid" name="country" value="<?=$countryid?>" size="100" readonly/><button type="button" onclick="selectCountry()" class="button_style_1">选择国家或地区</button>
            </td>
		</tr>
        <tr>
            <th width="120">
            ...
            <td>
                <?php foreach ($itemfields['trade'] as $k => $v): ?>
                    <label style="margin-left:10px;"><input type="checkbox" class="checkbox_style" value="<?=$k;?>" id="tradeid" name="tradeid[]"><?=$v['zh']?></label>
                <?php endforeach ?>
            </td>
        </tr>
        <tr>
            ...
            <td>
                <?php foreach ($itemfields['investmenttype'] as $k => $v): ?>
                    <label style="margin-left:10px;"><input type="checkbox" class="checkbox_style" value="<?=$k;?>" id="investmenttypeid" name="investmenttypeid[]"><?=$v['zh']?></label>
                <?php endforeach ?>
            </td>
        </tr>
        <tr>
        ...
        <tr>
            ...
            <td>
                <?php foreach ($itemfields['itemtype'] as $k => $v): ?>
                    <label style="margin-left:10px;"><input type="checkbox" class="checkbox_style" value="<?=$k;?>" id="itemtypeid" name="itemtypeid[]"><?=$v['zh']?></label>
                <?php endforeach ?>
            </td>
        </tr>
        <tr>
