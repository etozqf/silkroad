...
abstract class attribute_controller_abstract extends controller
{
	protected $app, $json, $template, $view, $config, $setting, $system, $_userid, $_username, $_groupid, $_roleid, $_departmentid;
    ...
    function __construct($app)
    {
		...
		$this->_departmentid = $app->departmentid;
    ...
    protected function priv_category($catid)
    {
		if (is_numeric($catid))
		{
			if (!priv::category($catid)) $this->showmessage("您没有<span style='color:red'>".table('category', $catid, 'name')."($catid)</span>地区权限！");
		}
		else
		{
			...
			if (is_array($catid))
			{
				foreach ($catid as $id)
				{
					if (!priv::category($id)) $this->showmessage("您没有<span style='color:red'>".table('category', $id, 'name')."($id)</span>地区权限！");
				}
			}
		}
    }
