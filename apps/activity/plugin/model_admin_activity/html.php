...
class plugin_html extends object 
{
	...
	{
		$this->write($this->activity->contentid);
		if ($this->activity->data['pagecount'] < $this->activity->data['old_pagecount'])
		{
			$this->unlink($this->activity->contentid, $this->activity->data['pagecount']+1, $this->activity->data['old_pagecount']);
		}
	}
	...
	private function write($contentid)
	{
        ...
        // 将字段中的验证规则转化为前端 JS 可用的正则
        loader::import('lib.activityField', app_dir('activity'));
        activityField::parseRules($r['fields'], true);
