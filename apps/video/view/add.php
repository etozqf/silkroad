    ...
    <tr>
        ...
        <td class="c_077ac7">
            <div id="videolist_info">
                <input id="listid" name="listid" type="hidden" value="0" />
...
<table width="98%" border="0" cellspacing="0" cellpadding="0" class="table_form mar_l_8">
    ...
    <tr>
        ...
        <td><textarea type="text" value="" name="yuanqu" placeholder="多个以；分隔" class=""></textarea></td>
        ...
        var ifm = ct.iframe({
            ...
            'ok':function(response, url) {
                if (response.state) {
                    ...
                    var thumb = response.content.thumb;
                    //判断如果缩略图没有后缀，那先可以本地化该张图片
                    if(['jpg','jpeg','png','bmp','gif'].indexOf(thumb.split('.').pop()) == -1){                    
                        $.post('?app=article&controller=article&action=thumb',{url:thumb},function(url){
                            $('[name="thumb"]').val(url);
                        });
                    }else{
                        $('[name="thumb"]').val(thumb);
                    }                                                                
...
$(function() {
    checkRepeat.init(<?php echo $repeatcheck; ?>);
});
//预览功能
var preview = function (contentid, catid, modelid) {
    $.post('?app=system&controller=content&action=preview', $('#video_add').serialize(), function (json){
        if (json.state) {
