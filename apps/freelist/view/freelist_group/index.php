...
row_template +='	<td class="t_r">{gid}</td>';
...
function common_edit(id,tr) {
	var url = '?app=<?=$app?>&controller=<?=$controller?>&action=edit&gid='+id;
	tr.trigger('check');
	ct.form('编辑分组',url,400,120,function(json){
		tableApp.updateRow(id, json.data);
		return true;
	}).dialog('option','modal',true);
}
function add_dialog() {
	ct.form('添加分组','?app=<?=$app?>&controller=<?=$controller?>&action=add',400,120,function(json){
