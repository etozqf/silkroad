...
class model_admin_edition extends model implements SplSubject 
{
	...
	function __construct()
	{
		...
		$this->_primary = 'eid';
		$this->_fields = array('eid', 'mid', 'title', 'number', 'total_number', 'image', 'pdf', 'year','url', 'publish', 'created','ip','createdby','disabled', 'description', 'ispay', 'premium');
	...
	public function save(array $data)
	{
		...
		$data = $this->filter_array($data, array('eid','mid', 'title', 'number', 'total_number','publish', 
												'created', 'image', 'pdf', 'ip','createdby','disabled', 'description', 'ispay', 'premium'));
		if($data['publish']) 
		{
			...
			if($data['year'] > 1900) 
			{
				$sql = "UPDATE #table_magazine SET default_year = ? WHERE mid = ? AND default_year < ?";
				$this->db->update($sql, array($data['year'], $data['mid'], $data['year']));
			}
		}
