...
<div class="table_head">
	...
	<? endforeach; ?>
	<input type="button" id="add" value="添加内容" class="button_style_4 f_l"/>
</div>
<div class="bk_8"></div>
<table width="98%" cellpadding="0" cellspacing="0" id="item_list" class="tablesorter table_list" style="margin-left:6px;">
  <thead>
    <tr>
	  ...
	  <th width="40">ID</th>
      <th class="ajaxer">标题</th>
      <th width="8%" class="ajaxer">
...
<div class="table_foot">
	...
	<input type="text" name="pagesize" value="<?=$size?>" size="2" id="pagesize"/> 条&nbsp;&nbsp;</div>
	<p class="f_l">
...
<tr id="tr_{eid}">\
	...
	<td class="t_c" size="20">{eid}</td>\
	<td name="title"  class="t_c" size="20">{title}</td>\
    <td class="t_c">{disabled_words}</td>\
    <td name="created"  class="t_c" size="30">{created}</td>\
    <td class="t_c count" style="display:none;"> <span>1</span> </td>\
...
$(function (){
	...
	ct.nav([
		{text: '扩展'},
		{text: '中英文刊物', url: '?app=<?=$app?>&controller=<?=$app?>&action=index'},
		{text: '<?=$head['title']?>'}
	]);
});
//init function
