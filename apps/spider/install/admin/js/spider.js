(function($){
    ...
    var buildNav = function(json)
    {
        ...
        var clockico = (json.cron == '1') ? '<div class="task_clock"></div>' : '';
        var li = $('<li id="task_'+taskid+'"><a href="javascript:;" title="'+json.title+'">'+json.title+'</a>'+clockico+'</li>').hover(function(){
            li.addClass('hover');
        },function(){
    ...
    var App = {
			...
			<h3 class="item-header">\
                ...
                rowReady:function(li, json){
                    li.find('img.marktask').click(function(e){
                        $(e.currentTarget).attr('src', 'images/add_1_disabled.gif');
                        e.stopPropagation();
                        taskAdd(json);
                    });
			...
			<td width="50" class="t_c">\
                ...
                rowReady:function(id, tr, json){
                    tr.find('img.del').click(function(e){
                        e.stopPropagation();
                        $('#item_'+id).find('.marktask').attr('src', 'images/add_1.gif');
                        task.remove(id);
                    });
                    var span = tr.find('span.catname').click(function(e){
                        e.stopPropagation();
                        e.preventDefault();
                        if (span.data('selectree')) return;
                        var input = span.find('input');
                        span.attr('data-catid', input.val() || '');
                        span.attr('data-value', json.catname || '请选择');
                        span.Selectree({
                            rootClass: 'category-datalist',
                            type: 'single',
                            checkParents: false,
                            checkedAfter: function(data) {
                                input.val(data.catid);
                            }
                        });
                    });
                },
        ...
        clearTask:function(){
            $(rightList).find('tr').each(function(i,k){
                var id = k.id.substr(5);
                $('#item_'+id).find('.marktask').attr('src', 'images/add_1.gif');
            });
            task.clear();
        },
        ...
        editTask:function(id, li, json){
            ct.form('编辑监控任务','?app=spider&controller=manager&action=editTask&taskid='+id,
                400,360,function(json){
                    ...
                    var checkbox, name;
                    $.each(form.find('[type="checkbox"]'), function(i, elm) {
                        checkbox = $(elm);
                        name = checkbox.attr('name');
                        if (!name) return true;
                        checkbox.removeAttr('name').data('name', name);
                        $('<input type="hidden">').insertAfter(checkbox).attr({
                            'name' : name,
                            'value' : ''
                        });
                    });
                    ...
                    $.each(form.find('[type="checkbox"]'), function(i, elm) {
                        ...
                        if (!name) return true;
                        form.find('[name="' + name + '"]').val(checkbox.is(':checked') ? checkbox.val() : '');
                    });
                });
        },
        ...
        run:function(){
            ...
            var buttonsRunning = ctrls.eq(2).show();
