<div class="bk_8"></div>
<div class="f_l w_670">
    <div class="tag_list_1 layout mar_t_10" style="padding-right: 1px;">
        <div class="f_14 b f_l">最近操作记录</div>
        <div class="f_r days">
            <?php foreach ($days as $key => $value): ?>
            <a href="#<?=($key + 1)?>"
               from="<?=($key + 1)?>"  <? if ($_GET['from'] == ($key + 1)) { ?>class="s_5"<? }?>><?=$value['name']?></a>
            <?php endforeach;?>
        </div>
    ...
    <table cellpadding="0" cellspacing="0" border="0" class="table_form mar_t_10" width="100%">
        ...
        <tr>
            ...
            <td><?=date('Y-m-d H:i:s', $page['published']);?><?php if ($page['updatedby']): ?>
                （<?= username($page['updatedby']) ?>）<?php endif;?></td>
        </tr>
        <?php endif;?>
        ...
        <tr>
            <th class="t_r">创建者：</th>
            <td><a href="javascript:url.member(<?=$page['createdby']?>);"><?=username($page['createdby']);?></a>
                （<?=date('Y-m-d H:i:s', $page['created']);?>）
            </td>
        </tr>
        <tr>
            <th class="t_r">区块数：</th>
            <td id="section_num">
                <script type="text/javascript">$('#section_num').text($('#section_list>li').length);</script>
            </td>
        </tr>
        <tr>
