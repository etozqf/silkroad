...
<textarea style="display:none;" id="tpl-form">
    <?=script_template('<form method="POST" name="section_recommend_edit_{%index%}" action="?app=page&controller=section_recommend&action=edit" class="section-form">
...
<textarea style="display:none;" id="tpl-row-check">
    <?=script_template('<div class="scroll-list-item" recommendid="{%recommendid%}">
        ...
        <p>
            ...
            <span><a href="javascript:void(0);" onclick="if (url && url.member) { url.member(\'{%userid%}\'); } else { return false; }">{%recommendedby_name%}</a></span>
        </p>
    </div>')?>
</textarea>
