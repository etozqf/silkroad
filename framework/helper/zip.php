...
class zip extends object 
{
    ...
    private function addZipItem ($zip, $racine, $dir)
    {
        if (is_dir($dir))
        {
            $zip->addEmptyDir(str_replace(str_replace('\\','/', $racine), '', str_replace('\\','/', $dir)));
            $lst = scandir($dir);
            unset($lst[array_search('.', $lst)]);
            unset($lst[array_search('..', $lst)]);
            foreach ($lst as $item)
                $this->addZipItem($zip, $racine, $dir.$item.(is_dir($dir.$item)?'/':''));
        }
        elseif (is_file($dir))
            $zip->addFile($dir, str_replace(str_replace('\\','/', $racine), '', str_replace('\\','/', $dir)));
    }
}