...
class tree extends object
{
	...
	function set($data)
	{
		if (isset($data[$this->field_parentid]) && (intval($data[$this->field_parentid]) < 1 || $data[$this->field_parentid] == $data[$this->field_id])) $data[$this->field_parentid] = null;
	...
	function set_each($data)
	{
		$ids = $parents = array($data[$this->field_id]);
		while ($parents) {
			$childs = array();
			foreach ((array)$this->get_child(array_pop($parents), $this->field_id . ',childids') as $item) {
				if (in_array($item[$this->field_id], $ids)) {
					continue;
				}
				$ids[] = $item[$this->field_id];
				if (!value($item, 'childids')) {
					continue;
				}
				foreach ((array)explode(',', $item['childids']) as $child) {
					if (empty($child)) {
						continue;
					}
					$ids[] = $child;
		...
		unset($data[$this->field_id], $data['childids'], $data['parentids']);
		$ret = $this->db->update("UPDATE `$this->table` SET `".implode('`=?,`', array_keys($data))."`=? WHERE `$this->field_id` IN (".implode(',', array_unique($ids)).")", array_values($data));
		return $ret;
	}
