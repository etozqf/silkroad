			...
			if ($this->seriesType==1){
			   ...
			   foreach($dataArray as $arrayvalue){
				 if(is_array($arrayvalue)){
				 	//edit by shanhuhai 2011/1/10
				   extract($arrayvalue, EXTR_PREFIX_ALL, 'pre_');
				   $tmparray = array_keys(array_slice($arrayvalue,2));
				   $params = '';
				   foreach ($tmparray as $t)
				   {
				   	  $params .= $t.'='.$arrayvalue[$t].';';
				   }
				   $this->addChartData($arrayvalue[1],"label=" . $arrayvalue[0].';'.$params );	 
				 }
			   } 		
			}else{
