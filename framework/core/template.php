...
class template extends view 
{
	...
	public function set_view($view, $app = null)
    {
		...
		$view = str_replace(array(':','*','?','"','<','>','|'), '-',
					str_replace(array('/', '\\'), ',', str_replace(ROOT_PATH, '', $this->file))) .'.php';
    	$this->compile_file = $this->compile_dir . $view;
        return $this;
    }
	...
	private static function _tag_parse($tag, $str, $end)
	{
		...
		$string .= $end ? ("$$return = \$_$tag;\n".'?>') : ("if (isset(\$_{$tag}['total'])): extract(\$_$tag, EXTR_OVERWRITE); \$_$tag = \$data;endif;\nforeach(\$_$tag as \$i=>\$$return): \$i++;\n".'?>');
		return $string;
	}
