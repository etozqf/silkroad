...
function tag_content_related($options)
{
    ...
    if (!isset($modelid))
    {
        $modelid = table('model');
        unset($modelid['3']);
        $modelid = implode_ids(array_keys($modelid));
    }
	$db = factory::db();
	$tagids = '';
	...
	return $db->select("SELECT DISTINCT t.contentid, c.title, c.thumb, c.url, c.published FROM `#table_content_tag` t LEFT JOIN `#table_content` c ON c.contentid=t.contentid WHERE $where ORDER BY c.published DESC LIMIT $size");
}
...
function tag_content($options)
{
	...
	{
		$fields	= 'c.contentid, c.catid, c.modelid, c.title, c.subtitle, c.color, c.thumb, c.tags, c.sourceid, c.url, c.weight, c.status, c.created, c.createdby, c.published, c.publishedby, c.allowcomment, c.comments, c.pv, c.spaceid, c.related, c.score, c.topicid';
	}
	$extrawhere = $where;
	$where = $join = $on = array();
	$where[] = "c.`status`=6";
	...
	else 
	{
        ...
        if (!empty($created))
        {
            ...
            {
                if (is_numeric($created) && strlen($created) < 4)
                {
                    $created = strtotime("-$created day");
                    $where[] = where_mintime('c.`created`', $created);
                }
                else
                {
                    $where[] = where_mintime('c.`created`', $created)." AND ".where_maxtime('c.`created`', $created);
                }
            }
            elseif(preg_match("/^\s*([\d]{4}\-[\d]{1,2}\-[\d]{1,2})?\s*\,\s*([\d]{4}\-[\d]{1,2}\-[\d]{1,2})?\s*$/", $created, $m))
            {
                if ($m[1]) $where[] = where_mintime('c.`created`', $m[1]);
                if ($m[2]) $where[] = where_maxtime('c.`created`', $m[2]);
            }
        }
    ...
    // MAKE SQL & DISTINCT
    /**
     * 当有两个以上的标签或属性时，并且contentid是第一字段时，对contentid去重处理
     */
    if(count($tagids) > 1 || count($proids) > 1)
    {
        if(stripos($fields,'contentid'))
        {
            $tmp_fields = str_ireplace('contentid', 'contentid', $fields);
            if((strpos($tmp_fields,'.contentid') && (strpos($tmp_fields, '.contentid') < 2)) || (strpos($tmp_fields,'contentid') === 0))
            {
                $fields = $tmp_fields;
            }
        }
    ...
    $sql = 'SELECT DISTINCT '.$fields.' FROM `#table_content` c ';
...
function tag_discuz($options)
{
    ...
    if (isset($version))
    {
        loader::import('lib.discuz.vendor', app_dir('system'));
        try
        {
            $discuz = discuz_vendor::instance($version, array('prefix' => $prefix));
            $tpost = $discuz->table_post;
            $tthread = $discuz->table_thread;
        }
        catch (Exception $ex)
        {
            return false;
...
function tag_db($options)
{
    ...
    if (isset($dsn) && is_array($dsn)
        && ($dsn['driver'] === 'mssql' || $dsn['driver'] === 'oci'))
    {
