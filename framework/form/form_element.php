...
class form_element
{
	...
	static function select($settings = array())
	{
		...
		{
			$selected = isset($value) && ($k == $value) ? 'selected="selected"' : '';
    ...
    static function intval($val, $default = '')
    {
        $val = intval($val);
        return $val ? $val : $default;
    }

    static function checked($val, $quote = '"')
    {
        return $val ? ' checked='.$quote.'checked'.$quote.' ' : ' ';
    }
