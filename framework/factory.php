...
class factory
{
    ...
    {
        if (!isset(self::$objects['datacache']))
        {
            import('cache.cache');
            $config = config('datacache');
            self::$objects['datacache'] =  new cache($config);
        }
        return self::$objects['datacache'];
    }
	...
	public static function db($config = 'db')
	{
        ...
        switch ($driver)
        {
            ...
            case 'oci':
                import('db.db_oci');
                return db_oci::get_instance($options);
                break;
            default:
                import('db.db');
	...
	public static function template($app = 'system')
	{
		...
		if (!isset(self::$objects[$key]))
		{
			...
			self::$objects[$key] = new template($config);
		}
	...
	public static function view($app = 'system')
	{
		...
		{
			import('core.view');
			self::$objects[$key] = new view(array('dir'=>app_dir($app).'view'.DS));
		}
		return self::$objects[$key];
	}
    ...
    {
    	if (!isset(self::$objects['nlp'])) {
    		$config = config('nlp');
    		import('helper.nlp');
    		self::$objects['nlp'] = new Nlp($config);
    	}
    	return self::$objects['nlp'];
    }
}